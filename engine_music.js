...
function recalculateDuration() {
    ...
    let den = parseInt(timeSigDen.value);
...
[bpmInput, timeSigNum, timeSigDen, measuresInput].forEach(el => {
    el.addEventListener('input', () => { recalculateDuration(); if(pInstance) loadAndRunSketch(); });
    el.addEventListener('change', () => { recalculateDuration(); if(pInstance) loadAndRunSketch(); });
});
...
uploadAudioInput.addEventListener('change', async (e) => {
    ...
    if (window.jsmediatags) {
        window.jsmediatags.read(file, {
            ...
            onError: function(error) {} // No tags, it's fine
        });
    }
    ...
    const offlineCtx = new OfflineAudioContext(
        ...
        Math.ceil(audioBuffer.duration * audioBuffer.sampleRate), 
...
btnExport.addEventListener('click', async () => {
    ...
    // Pre-encode embedded Audio Tracks immediately
    let audioEncoder = null;
    if (window.decodedAudioBuffer) {
        btnExport.innerText = 'Encoding Audio Track...';
        await new Promise(r => setTimeout(r, 10)); // Yield for UI

        audioEncoder = new AudioEncoder({
            output: (chunk, meta) => muxer.addAudioChunk(chunk, meta),
            error: e => console.error("WebCodecs Audio Encoding Error:", e)
        });
        ...
        const totalFramesAudio = window.decodedAudioBuffer.length;
        const sRate = window.decodedAudioBuffer.sampleRate;
        const chunkSize = sRate; // encode 1s chunks
        
        for (let offset = 0; offset < totalFramesAudio; offset += chunkSize) {
            const framesThisChunk = Math.min(chunkSize, totalFramesAudio - offset);
            const dataBuffer = new Float32Array(framesThisChunk * numChans);
            
            for(let c=0; c<numChans; c++) {
                const channelData = window.decodedAudioBuffer.getChannelData(c);
                dataBuffer.set(channelData.subarray(offset, offset + framesThisChunk), c * framesThisChunk);
            }
            ...
            const audioData = new window.AudioData({
                ...
                timestamp: (offset / sRate) * 1000000,
                data: dataBuffer
            });
    ...
    sketchParams.forEach(param => {
        ...
        if (param.type === 'slider') {
            ...
            if (param.canModulate !== false) {
                ...
                lfoHeader.style.flexDirection = 'column';
                ...
                modLabel.style.fontSize = '0.75em';
                modLabel.style.color = '#888';
                modLabel.innerText = 'Modulation:';
                
                const modSelect = document.createElement('select');
                modSelect.style.width = '120px';
                modSelect.style.padding = '2px';
                modSelect.style.border = '1px solid #333';
                modSelect.style.background = '#1a1a1a';
                modSelect.style.color = '#fff';
                modSelect.style.borderRadius = '3px';
                modSelect.style.fontSize = '0.75em';
                
                const modTypes = [
                    { value: 'none', text: 'None' },
                    { value: 'sine', text: 'Sine LFO' },
                    { value: 'square', text: 'Square LFO' },
                    { value: 'saw', text: 'Saw LFO' },
                    { value: 'triangle', text: 'Triangle LFO' },
                    { value: 'audio_freq', text: 'Audio: EQ Range' },
                    { value: 'audio_beat', text: 'Audio: Beat' }
                ];
                modTypes.forEach(t => {
                    const opt = document.createElement('option');
                    opt.value = t.value;
                    opt.innerText = t.text;
                    if ((param.lfo.enabled && param.lfo.mode === t.value) || (!param.lfo.enabled && t.value === 'none')) {
                        opt.selected = true;
                    }
                ...
                lfoHeader.appendChild(modRow);

                // Speed wrapper (only for LFOs)
                ...
                speedWrapper.style.justifyContent = 'flex-end';
                speedWrapper.style.marginTop = '4px';
                speedWrapper.innerHTML = `<span style="margin-right:4px;">LFO Speed:</span>`;
                ...
                eqWrapper.style.flexDirection = 'column';
                eqWrapper.style.marginTop = '4px';

                // Canvas for EQ
                const eqCanvas = document.createElement('canvas');
                eqCanvas.width = 180;
                eqCanvas.height = 30;
                eqCanvas.style.background = '#111';
                eqCanvas.style.borderRadius = '3px';
                eqCanvas.style.border = '1px solid #333';
                eqCanvas.style.width = '100%';
                
                // EQ Dual Slider
                const eqSliderContainer = document.createElement('div');
                eqSliderContainer.className = 'dual-slider-container';
                eqSliderContainer.style.marginTop = '-32px'; // overlay
                eqSliderContainer.style.height = '30px'; 
                
                const eqTrack = document.createElement('div');
                eqTrack.style.position = 'absolute';
                eqTrack.style.width = '100%';
                eqTrack.style.height = '100%';
                eqSliderContainer.appendChild(eqTrack);
                
                const eqHighlight = document.createElement('div');
                eqHighlight.style.position = 'absolute';
                eqHighlight.style.height = '100%';
                eqHighlight.style.background = 'rgba(102, 252, 241, 0.2)'; // tint the selected region
                eqHighlight.style.borderLeft = '1px solid #66FCF1';
                eqHighlight.style.borderRight = '1px solid #66FCF1';
                eqSliderContainer.appendChild(eqHighlight);
                
                param.lfo.audioMinFreq = param.lfo.audioMinFreq || 0;
                param.lfo.audioMaxFreq = param.lfo.audioMaxFreq || 63;
                
                const eqMinIn = document.createElement('input');
                eqMinIn.type = 'range'; eqMinIn.min = 0; eqMinIn.max = 63; eqMinIn.step = 1; eqMinIn.value = param.lfo.audioMinFreq;
                eqMinIn.style.zIndex = '4'; eqMinIn.style.height = '30px'; eqMinIn.style.opacity = '0';
                
                const eqMaxIn = document.createElement('input');
                eqMaxIn.type = 'range'; eqMaxIn.min = 0; eqMaxIn.max = 63; eqMaxIn.step = 1; eqMaxIn.value = param.lfo.audioMaxFreq;
                eqMaxIn.style.zIndex = '5'; eqMaxIn.style.height = '30px'; eqMaxIn.style.opacity = '0';
                
                eqSliderContainer.appendChild(eqMinIn);
                eqSliderContainer.appendChild(eqMaxIn);
                
                const updateEqHighlight = () => {
                    const minP = param.lfo.audioMinFreq / 63 * 100;
                    const maxP = param.lfo.audioMaxFreq / 63 * 100;
                    eqHighlight.style.left = minP + '%';
                    eqHighlight.style.width = (maxP - minP) + '%';
                };
                updateEqHighlight();
                
                eqMinIn.addEventListener('input', (e) => {
                    let val = parseInt(e.target.value);
                    if(val > param.lfo.audioMaxFreq) { val = param.lfo.audioMaxFreq; eqMinIn.value = val; }
                    param.lfo.audioMinFreq = val; updateEqHighlight();
                });
                eqMaxIn.addEventListener('input', (e) => {
                    let val = parseInt(e.target.value);
                    if(val < param.lfo.audioMinFreq) { val = param.lfo.audioMinFreq; eqMaxIn.value = val; }
                    param.lfo.audioMaxFreq = val; updateEqHighlight();
                });

                eqWrapper.appendChild(eqCanvas);
                eqWrapper.appendChild(eqSliderContainer);
                lfoHeader.appendChild(eqWrapper);
                
                // Live EQ render loop attached to this param
                const eqCtx2D = eqCanvas.getContext('2d');
                const drawEQ = () => {
                    if (param.lfo.mode === 'audio_freq' && window.audioAnalysisData && window.audioAnalysisData[currentFrame]) {
                        const bins = window.audioAnalysisData[currentFrame].bins;
                        if (bins) {
                            eqCtx2D.clearRect(0,0, eqCanvas.width, eqCanvas.height);
                            const w = eqCanvas.width / 64;
                            for(let i=0; i<64; i++) {
                                const h = bins[i] * eqCanvas.height;
                                eqCtx2D.fillStyle = (i >= param.lfo.audioMinFreq && i <= param.lfo.audioMaxFreq) ? '#66FCF1' : '#555';
                                eqCtx2D.fillRect(i*w, eqCanvas.height - h, w, h);
                            }
                        }
                    }
                    if (document.getElementById(`param-${param.id}`)) { // still in DOM
                ...
                modSelect.addEventListener('change', (e) => {
                    ...
                    param.lfo.enabled = (mode !== 'none');
                    param.lfo.mode = mode || 'sine';
                    
                    if (['sine', 'square', 'saw', 'triangle'].includes(mode)) {
                        speedWrapper.style.display = 'flex';
                        eqWrapper.style.display = 'none';
                    } else if (mode === 'audio_freq') {
                        speedWrapper.style.display = 'none';
                ...
                if (param.lfo.enabled) { // trigger initial state config
            ...
            if (activeSketch && activeSketch.parameters) {
                activeSketch.parameters.forEach(param => {
                    if (param.lfo && param.lfo.enabled && param.type === 'slider') {
                        let modPhase = 0;
                        const mode = param.lfo.mode || 'sine';
                        
                        if (['sine', 'square', 'saw', 'triangle'].includes(mode)) {
                            const lfoProgress = (progress * param.lfo.speed) % 1.0;
                            
                            if (mode === 'sine') {
                                modPhase = (Math.sin(lfoProgress * Math.PI * 2) + 1) / 2;
                            } else if (mode === 'square') {
                                modPhase = lfoProgress < 0.5 ? 1 : 0;
                            } else if (mode === 'saw') {
                                modPhase = lfoProgress;
                            } else if (mode === 'triangle') {
                                modPhase = lfoProgress < 0.5 ? lfoProgress * 2 : 2 - (lfoProgress * 2);
                            }
                        } else if (mode === 'audio_freq' || mode === 'audio_beat') {
                            if (window.audioAnalysisData && window.audioAnalysisData[currentFrame]) {
                                const ad = window.audioAnalysisData[currentFrame];
                                if (mode === 'audio_freq' && ad.bins) {
                                    let sum = 0;
                                    let count = 0;
                                    const minF = param.lfo.audioMinFreq || 0;
                                    const maxF = param.lfo.audioMaxFreq || 63;
